...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.thisTableView deselectRowAtIndexPath:self.thisTableView.indexPathForSelectedRow animated:YES];
...
- (void)dealloc {
    ...
    self.thisTableView.delegate = nil;
    self.thisTableView.dataSource = nil;
...
- (void)viewWillLayoutSubviews {
    ...
    self.thisTableView.scrollIndicatorInsets = UIEdgeInsetsMake(self.tableViewInsertTop, 0, self.tableViewInsertBottom, 0);
...
- (void)setRefreshBlock:(void (^)())refreshBlock {
    ...
    if (self.thisTableView) {
        self.thisTableView.tableHeaderView = self.tableHeaderView;
    }
...
- (void)beginRefresh {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.15 delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            self.thisTableView.contentInsetTop = kRefreshHeight + self.tableViewInsertTop;
...
- (void)endRefresh {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        self.thisTableView.contentInsetTop = self.tableViewInsertTop;
    }];
...
- (void)beginLoadMore {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [UIView animateWithDuration:0.15 delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            self.thisTableView.contentInsetBottom = kRefreshHeight + self.tableViewInsertBottom;
...
- (void)endLoadMore {
    ...
    [UIView animateWithDuration:0.2 animations:^{
        self.thisTableView.contentInsetBottom =  + self.tableViewInsertBottom;
    }];
...
- (void)setLoadMoreBlock:(void (^)())loadMoreBlock {
    ...
    if (self.loadMoreBlock && self.thisTableView) {
        self.thisTableView.tableFooterView = self.tableFooterView;
    }
